...
vec2 smoothDist(in vec3 pos) {
    AnimatedBranch main = toBranch(branchs[instanceID]);
...
vec3 calcNorm(in vec3 pos, in float t, float tDif, in vec3 normal, in Bezier main) {
    if(t < tDif) {
        ...
        float widthAtTdif = branchRadius(tDif, main.endT, main.lowRadius, main.highRadius);
        ...
        if(bez.start == main.end) {
            ...
            float widthAtTdif = branchRadius(tDif, main.endT, bez.lowRadius, bez.highRadius);
...
float calc_uvx(AnimatedBranch branch, float clampedT, vec3 curPos) {
...
vec2 calc_uv(AnimatedBranch branch, float clampedT, vec3 curPos, float v) {
...
Hit intersect(vec3 pos, vec3 rayDir, in AnimatedBranch branch, float maxStepCount, float minDist) {
    ...
    if(splineT != -100.0) {
		vec3 curPos = pos + rayDir * t;
        float clampedT = clamp(splineT, 0.0, branch.endT);
        ...
        onSphere = splineT > branch.endT || splineT < 0.0;
        splineT = clampedT;
    }
